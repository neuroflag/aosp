...
char *firefly_get_position(void)
{
#define ARRAY_SIZE(x) (sizeof(x)/sizeof(x[0]))
#define CORE_ADC_PATH "/sys/bus/iio/devices/iio:device0/in_voltage1_raw"
#define SUB_POSITION_PATH "/dev/sub_position"   
       int fd,fd1;
       char adcstr[5];
       int adcvalue;
       ssize_t nbytes;
       ...
       char serial[6];
       int sub_position = 0;
       int sub_position_list[] = {6, 3, 0, 7, 4, 1, 8, 5, 2};
...
//               {-1,  20},
       ...
       char prop_cluster_server[PROPERTY_VALUE_MAX];

       fd = open(CORE_ADC_PATH, O_RDONLY);//read form buffered file
       if(fd>0) {
               property_get("ro.firefly.server.version", prop_cluster_server, "");
               if(!strcmp(prop_cluster_server, "R2")) {
                       //Firefly CLUSTER-SERVER-R2.
                       fd1 = open(SUB_POSITION_PATH, O_RDONLY);
                       if(fd1>0)
                       {
                               read(fd1, &sub_position, 1);
                               SLOGE("sub_position = %d", sub_position);
                               close(fd1);
                       }
                       else {
                               SLOGE(" open file %s err", SUB_POSITION_PATH);
                               return NULL;
                       }
                       for (j = 0; j < ARRAY_SIZE(sub_position_list); j++ ) {
                               if (sub_position_list[j] == sub_position)
                                       break;
                       }
                       nbytes = read(fd, adcstr, sizeof(adcstr) - 1);
                       adcvalue = atoi(adcstr);
                       for (i=0; i<ARRAY_SIZE(adctab_u2); i++) {
                               if ((adcvalue >= adctab_u2[i][0]) && (adcvalue < adctab_u2[i][1])) {
                                       sprintf(serial, "S%02d%02d", j+1, i+1);
                                       break;
                               }
                       }
                       if (i == ARRAY_SIZE(adctab_u2))
                               sprintf(serial, "S%2dXX", j+1);
               } else if (!strcmp(prop_cluster_server, "R1")) {
                       //Firefly CLUSTER-SERVER-R1.
                       nbytes = read(fd, adcstr, sizeof(adcstr) - 1);
                       adcvalue = atoi(adcstr);
                       for (i=0; i<ARRAY_SIZE(adctab_u1); i++) {
                               if ((adcvalue >= adctab_u1[i][0]) && (adcvalue < adctab_u1[i][1])) {
                                       if ((i==0) || (i==11))
                                               strcpy(serial, "0MAIN");
                                       else
                                               sprintf(serial, "SUB%02d", i);
                                       break;
                               }
                       }
                       if (i == ARRAY_SIZE(adctab_u1))
                               strcpy(serial, "SUBXX");
               }
               ...
               SLOGE(" open file %s err", CORE_ADC_PATH);
               return NULL;
       }
}
...
#ifdef ENABLE_GENERATE_REGULAR_ADB_DEVICE_NAME
       char *xxid, *position_p;
       char prop_server_serialno[PROPERTY_VALUE_MAX], position[PROPERTY_VALUE_MAX];
       property_get("ro.firefly.server.serialno", prop_server_serialno, "");
       if(!strcmp(prop_server_serialno, "true"))
       {
               SLOGE("prop_server_serialno=%s",prop_server_serialno);

               xxid = firefly_get_xxid();
               position_p = firefly_get_position();
               if ((xxid != NULL) && (position_p != NULL))
               {
                       strncpy(result, xxid, 5);
                       strcat(result, position_p);
                       store_serialno(result);
                       for(int i=0; i < 6; i++)
                               position[i] = position_p[i];
                       property_set("ro.board.position", position[0] ? position : "");
                       return;
               }
       }
#endif
