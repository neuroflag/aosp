...
static void startLedBlink(int led);
static void LedOn(int led);
static void LedOff(int led);
static void stopLedBlink(int led);
// add end
...
int do_sd_mode_update(const char *pFile, SDBoot* prksdboot, RecoveryUI* ui){
    // add by firefly
    char firefly_recovery_script[256]={0};
    // add end
    ...
    if(bRet && !access(firefly_recovery_script, 0)) { //run firefly scripts
      ...
      char *cmd[4];
      char *envp[2];
      envp[0]=strdup("PATH=/sbin");
      envp[1]=NULL;
      cmd[0] = strdup("/sbin/busybox");
      cmd[1] = strdup("ash");
      cmd[2] = strdup(firefly_recovery_script);
      cmd[3] = NULL;
      bRet = exec_firefly_cmd(cmd[0], cmd, envp) == 0;
      printf("firefly-recovery.sh ret=%d\n", bRet);
      ui->Print("firefly-recovery.sh ret=%d\n", bRet);
    }
    // add end
...
int exec_firefly_cmd(const char* path, char* const argv[], char* const envp[]) {
    int status;
    pid_t child;
    if ((child = vfork()) == 0) {
