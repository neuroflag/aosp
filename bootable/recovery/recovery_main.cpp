...
{
      int system_ret = -5;
      //printf("Running %s", "/sbin/resize_userdata.sh");
      char *cmd[4];
      char *envp[3];
      envp[0]=strdup("PATH=/sbin:/system/bin/");
      envp[1]=NULL;
      cmd[0] = strdup("/sbin/busybox");
      cmd[1] = strdup("ash");
      cmd[2] = strdup("/sbin/resize_userdata.sh");
      cmd[3] = NULL;
      system_ret = exec_firefly_cmd(cmd[0], cmd, envp) == 0;
      //printf("resize_userdata.sh ret=%d\n", system_ret);
}
