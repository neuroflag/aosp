...
HWC2::Error DrmHwcTwo::HwcDisplay::SetPowerMode(int32_t mode_in) {
  ...
  fd_fb_firefly = open("/dev/fb_firefly", O_RDWR, 0);
  if (fd_fb_firefly < 0) {
    ALOGE("Open fb_firefly fail in %s,error=%d\n",__FUNCTION__,fd_fb_firefly);
  }
  ...
  if (fb_blank != fb_blanked && fd_fb_firefly > 0) {
    int err = ioctl(fd_fb_firefly, FBIOBLANK, fb_blank);
    if (err < 0) {
      ALOGE("fb_firefly ioctl failed=%d,fb_blank = %d dpms_value=%" PRIu64 " \n",err,fb_blank,dpms_value);
    }
  }
